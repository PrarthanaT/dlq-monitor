...
class RetryEngine:
    ...
    async def retry_message(self, message: DLQMessage) -> RetryResult:
        ...
        try:
            ...
            return RetryResult(message_id=message.message_id, correlation_id=message.correlation_id, success=True, attempt=attempt)
        except Exception as exc:
            log.error("retry_failed", attempt=attempt, error=str(exc))
            return RetryResult(message_id=message.message_id, correlation_id=message.correlation_id, success=False, attempt=attempt, error=str(exc))
